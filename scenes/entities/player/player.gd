...
func move_logic(delta) -> void:
	...
	if movement_input != Vector2.ZERO:
		...
		var smoothed := vel_2d.lerp(desired_vel, 0.2) * speed_modifier # 0.2 -> 0.5 for more snap
		
		velocity.x = smoothed.x
		velocity.z = smoothed.y
		skin.set_move_state("Running")
		var target_angle := -movement_input.angle() + PI/2
		skin.rotation.y = rotate_toward(skin.rotation.y, target_angle, 6 * delta)
